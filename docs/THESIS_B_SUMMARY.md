...
## 9. 🔴 Trivial-baseline control (2026-09-07) — ผลที่ต้องรายงานตรง ๆ

หลังพบว่าการประเมินระดับจังหวัดขาด **control ที่จำเป็น** (กฎง่าย ๆ ที่ไม่ใช้กราฟเลย) เราเพิ่มเข้าไป
([`src/eval/attribution_audit.py`](../src/eval/attribution_audit.py)) ผลคือ:

| กติกาเตือน | POD | FAR | CSI |
|---|---|---|---|
| **causal gate (ระบบ B ปัจจุบัน)** | 0.810 | 0.218 | **0.660** |
| **TRIVIAL: เตือนทุกจังหวัด ยกเว้นที่มีคันกั้นน้ำ** | **0.988** | **0.210** | **0.783** |
| TRIVIAL: เตือนทุกจังหวัด | 1.000 | 0.270 | 0.730 |

→ **ระบบเตือนของเราแพ้กฎ trivial ทุกเมตริก** (CSI 0.660 < 0.783 · POD 0.810 < 0.988 · FAR ก็ไม่ดีกว่า)

ประกอบกับผลฝั่ง A: MCC ของ causal gate (+0.203) มาจาก**กฎคันกั้นน้ำล้วน ๆ** — พอตัดกรุงเทพ/นนทบุรีออก
เหลือ **MCC −0.048 (แย่กว่าเดาสุ่ม)** และ permutation test ยืนยันว่า**สับสายกราฟมั่วก็ได้ผลเท่ากัน (p=0.28)**

**ข้อสรุปของ Plan B (ซื่อสัตย์):** ที่ความละเอียดระดับจังหวัด ซึ่ง base rate สูงถึง 0.73
**ระบบเตือนนี้ไม่มีคุณค่าเชิงปฏิบัติการเหนือกฎง่าย ๆ** — ตัวเลข CSI 0.660 ที่เคยดูดีเป็นผลของ base rate ไม่ใช่ skill
จึง **ห้าม claim ว่าเป็น operational early-warning** ต้องรายงานเป็นข้อจำกัดหลักของส่วนขยายนี้
(ทางแก้ที่ทดสอบแล้วว่าเป็นไปได้ = ลงความละเอียดระดับอำเภอ ซึ่ง base rate ลดเหลือ 0.20–0.26)
