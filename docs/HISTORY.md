...
### 2026-09-06 (ต่อ) — ทดสอบ soil moisture (ERA5-Land) เพื่อยกเพดาน → ผลลบ (ปฏิเสธอย่างซื่อสัตย์)

ลองลีเวอร์ที่ชัดสุดในการยกเพดาน: **root-zone soil moisture (ERA5-Land 7–28cm, open-meteo archive)** —
สมมติฐาน: FN (น้ำท่วมที่โมเดลพลาด) มาจากดินอิ่มตัว. เช็ก percentile ของ soil moisture ช่วงเหตุการณ์ vs
climatology 1991–2020 ของจังหวัด FN (2566: 5 จว. · 2567: 10 จว.):
- **median FN percentile — 2566: 28% · 2567: 38%** (ไม่อิ่มตัว) · นครสวรรค์ **0%/7%** (ดินแห้งมากแต่ท่วม),
  อยุธยา 3%/37%, ชัยนาท 23%/40%, อ่างทอง 20%.
- → **จังหวัดที่พลาด (โดยเฉพาะสายหลัก) มีดินแห้งถึงปกติ ไม่ใช่ดินอิ่มน้ำ** → น้ำท่วม**ไม่ได้มาจาก saturation**
  แต่มาจาก **กลไกชลศาสตร์** (confluence backwater / น้ำล้นที่ราบต่ำกว่าหมุดตลิ่งช่องหลัก / การจัดการน้ำ).

**ข้อสรุป: soil moisture ไม่ช่วย** (จะเพิ่มแต่ FP บนจังหวัดที่ไม่ใช่ FN → MCC ไม่ขึ้น, spec ลง) → **ไม่ใส่เข้าโมเดล**.
ยืนยันเพดาน ~0.20 เป็นเพดานข้อมูล และระบุชัดว่า **residual floods = hydraulic/backwater** ที่สัญญาณจุดทุกตัว
(ระดับน้ำ/ฝน/ความชื้นดิน) มองไม่เห็น → ต้อง hydrodynamic model (HEC-RAS) หรือเกจถี่ขึ้นที่จุดบรรจบ (future work).

### 2026-09-07 — 🔴 ผลชี้ขาด: causal topology ไม่ได้ช่วยการทำนายเลย (การทดลอง 2 ชุด)

ทำตาม roadmap P0 (structure ablation) + การทดลอง mechanism agreement ที่เสนอเพิ่ม. ผลออกมาเป็น
**ผลลบที่หนักแน่นและสำคัญที่สุดของโปรเจกต์**.

**(1) Structure ablation — permutation test** ([`src/eval/structure_ablation.py`](../src/eval/structure_ablation.py))
สับสาย province→reach แบบสุ่ม (คง node/evidence/สัญญาณเท่าเดิม = information-matched) 2,000 ครั้ง:
| test | observed MCC | null mean | p |
|---|---|---|---|
| A fluvial topology | +0.157 | +0.142 | **0.55** |
| B full model | +0.203 | +0.173 | **0.28** |
| C pluvial locality | +0.121 | +0.097 | **0.40** |
→ **กราฟที่สับสายมั่ว ทำได้พอ ๆ กับกราฟเหตุ-ผลจริง** (ไม่มีนัยสำคัญเลยสักเทสต์)

**(2) Trivial-baseline control** ([`src/eval/attribution_audit.py`](../src/eval/attribution_audit.py))
| variant | MCC | F1 |
|---|---|---|
| causal gate (23 จว.) | +0.203 | 0.795 |
| **"ท่วมหมด ยกเว้นจังหวัดมีคันกั้นน้ำ"** | **+0.438** | 0.878 |
| ท่วมหมด (รวมจังหวัดมีคันกั้น) | 0.000 | 0.844 |
| **causal gate (เฉพาะ 21 จว. ที่ไม่มีคันกั้น)** | **−0.048** | 0.800 |
→ **MCC ทั้งหมดของระบบมาจากกฎ "คันกั้นน้ำ" ไม่ได้มาจาก causal graph** และบนจังหวัดที่ท่วมได้จริง
gate ของเรา **แย่กว่าการเดาสุ่ม (−0.048)** · กฎง่าย ๆ ที่ไม่ใช้กราฟเลยชนะระบบเราเท่าตัว

**(3) Mechanism agreement — "ถูกด้วยเหตุผลผิด"**
TP 68 เคส: corroborated (≥2 กลไก) **44%** · กลไกเดียว **56%** · **อาศัยแค่ฝน 30 วัน (สัญญาณไม่เฉพาะเจาะจง) 21%**
· กลไกเดียว+เฉียดเกณฑ์ (<20%) **46%**. เคสสระบุรีตรงตามที่ตั้งข้อสงสัย:
2564/2565 fluvial แรง (margin 0.96/1.07) แต่ **2566 เหลือแค่ฝน 30 วัน (0.23) · 2567 แค่ 0.08** และ **2568 เป็น FP**
→ ปีที่ผู้เชี่ยวชาญระบุว่าเป็น**น้ำป่า** ระบบกลับให้เหตุผลว่าเป็นฝนสะสมทั้งฤดู = correctness โดยไม่มี faithfulness

**(4) ทดสอบเติมกลไกน้ำป่า → ทำไม่ได้ด้วยข้อมูลที่มี** ฝนรายชั่วโมง ERA5 ที่สระบุรี/มวกเหล็ก ปี 2566–2567
(max1h 19.8/20.4, max6h 41.4/37.5) **ไม่มีลายเซ็นน้ำป่า** — แต่เป็นเพราะ ERA5 กริด ~9 กม. เกลี่ยพายุฝนเฉพาะจุดทิ้ง
ต้องใช้เรดาร์/เกจฝนรายชั่วโมงหนาแน่น ซึ่งเข้าไม่ถึงย้อนหลัง.

**ข้อสรุป:** ในระดับจังหวัดด้วย gold ชุดนี้ **การทำนายเกือบเป็นเรื่อง trivial** (base rate 0.73) และ causal graph
**ไม่มีอำนาจแยกแยะเชิงพื้นที่**. สิ่งที่เหลือรอดคือ *การอธิบาย/ตรวจสอบย้อนกลับ* และ *ระเบียบวิธีประเมิน* ไม่ใช่ความแม่นยำ.

### 2026-09-07 (ต่อ) — Plan A re-scope (A): ลงความละเอียดระดับอำเภอ + Plan B honesty control

**Plan B (ทำนายต่อเหมือนเดิม) — เพิ่ม trivial-baseline control ที่ขาดไป:**
| กติกาเตือน | POD | FAR | CSI |
|---|---|---|---|
| causal gate (B ปัจจุบัน) | 0.810 | 0.218 | 0.660 |
| **TRIVIAL: เตือนทุกจังหวัด ยกเว้นมีคันกั้นน้ำ** | **0.988** | **0.210** | **0.783** |
| TRIVIAL: เตือนทุกจังหวัด | 1.000 | 0.270 | 0.730 |
→ **B แพ้กฎ trivial ทุกเมตริก** ที่ระดับจังหวัด → ห้าม claim operational warning (บันทึกใน THESIS_B_SUMMARY §9)

**Plan A re-scope (A) — ระดับอำเภอ:**
- gold: GISTDA ระดับตำบล (thaiwater YearlyReport 2566/2567) รวมเป็นอำเภอ, cutoff ≥10,000 ไร่ (เท่าเดิม)
- **base rate 0.73 (จังหวัด) → 0.239 (อำเภอ)** · 266 อำเภอ × 2 เหตุการณ์ = **532 เคส** (เดิม 115)
- จับคู่ GADM level-2 ได้ 266/280 (14 อำเภอตั้งใหม่หลัง GADM 4.1 = ข้อจำกัด)
- **บั๊กที่เจอ:** GADM 4.1 ใส่ `NL_NAME_1` ของกรุงเทพเป็น "จังหวัดเชียงใหม่" → เดิมยัด 50 เขต กทม. ไปเป็นเชียงใหม่
  แก้โดย match ด้วยชื่อโรมัน `NAME_1`

**ผลเบื้องต้นระดับอำเภอ (ยังไม่มีฝนรายอำเภอ):**
| variant | MCC | F1 | recall | spec |
|---|---|---|---|---|
| fluvial อย่างเดียว (สืบทอดจากจังหวัด) | +0.131 | 0.229 | 0.157 | 0.931 |
| trivial: เดาท่วมหมด | 0.000 | 0.385 | 1.000 | 0.000 |
| **trivial: หมด ยกเว้นมีคันกั้นน้ำ** | **+0.289** | 0.464 | 1.000 | 0.277 |

→ fluvial สืบทอดจากจังหวัดจึง **แยกแยะภายในจังหวัดไม่ได้เลย** (recall 0.157) และยังแพ้ trivial
→ **ตัวชี้ขาดคือฝนรายอำเภอ (ERA5 ต่อ centroid)** ซึ่งกำลังดึงอยู่ (ติด hourly API limit ต้องทยอย)
