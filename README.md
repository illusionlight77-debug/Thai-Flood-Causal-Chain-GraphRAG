...
GISTDA STAC API ยังต่อไม่ติด และ **Sentinel-1 ผ่าน Google Earth Engine ทำไม่ได้** (ต้องมี GEE account/OAuth
ซึ่ง environment นี้ไม่มี — ดู Bugs). จึงใช้ **ผลวิเคราะห์ภาพดาวเทียม GISTDA ของเหตุการณ์ NORU 28 ก.ย.–14 ต.ค. 2565**
...
**ทำไมยังไม่ปิด:** ต้อง (1) ข้อมูลพื้นที่ท่วมรายจังหวัดปี 2564 ให้ครบทั้ง 10 จังหวัดในกราฟ (ตอนนี้บางจังหวัดถูกรวมกลุ่ม),
(2) สถานะเขื่อนปี 2564 จริง (ปีนั้นเพิ่งพ้นภัยแล้ง เขื่อนใหญ่ยังไม่เต็ม), (3) **refactor ให้ pipeline เลือก event ได้**
(ตอนนี้ fixtures ผูกกับ 2565). เป็นงานขนาดเท่า 1 phase → กันไว้เป็นงานรอบถัดไป **ไม่ทำครึ่ง ๆ / ไม่กุตัวเลข**.
ดังนั้นผลด้านบนยัง = **1 เหตุการณ์** → generalization **ยังพิสูจน์ไม่ได้** (ระบุใน Limitations).
